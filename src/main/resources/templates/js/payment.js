window.onload = () => {
    ...
    // ✅ 포트원 초기화 (이 시점에 IMP는 100% 로드됨)
    IMP.init("imp77335438");

    // 구독 상태 조회
    fetch(`http://localhost:8081/billing/info?customerUid=${customerUid}`)
        .then((res) => res.json())
        .then((data) => {
            const { isActive, billingStartDate, cardName } = data;
        ...
        `;
                ...
                btn.addEventListener("click", () => {
                    IMP.request_pay({
                        pg: "tosspayments.iamporttest_4", // 테스트용 PG사
                        pay_method: "card",
                        customer_uid: customerUid,
                        name: "정기결제 카드 등록",
                        amount: 100, // 테스트는 최소 100원
                        buyer_email: "test@test.com",
                        buyer_name: "홍길동",
                        buyer_tel: "01012345678"
                    }, function (rsp) {
                        if (rsp.success) {
                            fetch(`http://localhost:8081/billing/register?impUid=${rsp.customer_uid}&customerUid=${rsp.customer_uid}`, {
                                method: "POST"
                            })
                                .then(res => res.json())
                                .then(data => {
...
function cancelSubscription() {
    ...
    fetch(`http://localhost:8081/billing/cancel?customerUid=${customerUid}`, {
        method: "POST"
    })
        .then(res => res.text())
        .then(msg => {
